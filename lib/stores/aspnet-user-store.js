...
function AspNetUserStore() {
    ...
    self.on('aspnet_roles_fetch', function() {
        ...
        if(self.systemRoles){
            self.trigger('aspnet_roles_changed',self.systemRoles);
        }else{
            var url = baseAspNetUrl + 'roles';
            RiotControl.trigger('fetch',url,null,{name:'private_aspnet_roles_changed'});
        }
    })

    RiotControl.on('private_aspnet_roles_changed', function(data){
        console.log('private_aspnet_roles_changed:',data);
    ...
    self.on('aspnet_roles_create', function(query) {
        ...
        var url = baseAspNetUrl + 'roles/create?roleName=' + query.role;
        RiotControl.trigger(
            'fetch',
            url,
            {
            ...
            {name:'aspnet_roles_fetch'});
    })


    self.on('aspnet_roles_delete', function(query) {
        console.log('aspnet_roles_create:');
        var url = baseAspNetUrl + 'roles/delete?roleName=' + query.role;
        RiotControl.trigger(
            'fetch',
            url,
            {
